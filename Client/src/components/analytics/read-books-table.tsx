...
// Initial sample data
const initialRecord: Checkin[] = [];

// Define the table columns
const columns: ColumnDef<Checkin>[] = [
  {
    accessorKey: "checkin_at",
    header: ({ column }) => (
      <Button
        variant="ghost"
        onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
      >
        Checked In Time <ArrowUpDown className="ml-2 h-4 w-4" />
      </Button>
    ),
    cell: ({ row }) => (
      <div className="font-medium">{row.getValue("checkin_at")}</div>
    ),
  },
  {
...
export default function ReadBooksTable() {
  ...
  const table = useReactTable({
    data: checkinResult?.data ?? initialRecord,
    columns,
    state: {
      sorting,
      columnFilters,
